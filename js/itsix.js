$(function () {
    //logo 클릭시 최상단으로 이동
    // "click", function = 로고 클릭시 함수 실행
    // scrollTop은 스크롤 위치 이동을 위한 jQuery 애니메이션 함수
    ...
    // PC + Mobile 메뉴 클릭시 에니메이션으로 위치 이동
    ...
    $(window).scroll(function () {
        ...
        $("ul.center_bg li").eq(0).css({
            "top": -100 + (scr / 2) // -100은, 기존의 요소에서 상단으로 100픽셀 올려준 상태 // (scr / 2) : 현재 스크롤 위치를 2로 나눈 값을 의미
            // 스크롤 위치에 따라 동적으로 계산된 값에 따라 위치 이동이 이루어짐 // 때문에 스크롤 위치가 증가하면 (scr / 2)값도 증가,
            // 스크롤 위치가 감소하면 (scr / 2)값도 감소
        });
